...
AV_X_MSE = round(sum(X_MSE_AV) / 1122,3)
AV_X_MAE = round(sum(X_MAE_AV) / 1122,3)
AV_Y_MSE = round(sum(Y_MSE_AV) / 1122,3)
AV_Y_MAE = round(sum(Y_MAE_AV) / 1122,3)
print(AV_X_MSE)
print(AV_X_MAE)
print(AV_Y_MSE)
print(AV_Y_MAE)

d = (x_error**2+y_error**2)**1/2
y_test = pd.Series(y_test)
x_predict = pd.Series(x_predict)
y_test = pd.Series(y_test)
y_predict = pd.Series(y_predict)
d = pd.Series(d)
massiv = pd.DataFrame({'x': x_test,'y': y_test,'d': d})
# график x_err и y_err в зависимости от x и y
plt.figure()
plt.grid(True)
