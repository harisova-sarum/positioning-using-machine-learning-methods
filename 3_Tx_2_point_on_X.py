...
df = df.drop(df[(df.x==5)].index)
df = df.drop(df[(df.x==9)].index)
df = df.drop(df[(df.x==15)].index)

df = pd.concat([pd.DataFrame(pd.concat([t.iloc[0], t.iloc[1]])).transpose()
      for t in np.array_split(df, df.shape[0] / 2)])
